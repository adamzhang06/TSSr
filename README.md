...
Considering that soft-clipping during mapping of sequencing reads to reference genome could also exclude non-matching 5’end bases other than unmatched Gs, which introduces false positive TSSs. Therefore, we’d recommend not to use soft-clipping for sequencing data. In the case that soft-clipped bam files are used, the users should set "softclippingAllowed = TRUE" for getTSS function. Additionally, the "useMultiCore" parameter can be set to "TRUE" to utilize more cores of your machine to speed up the process of "getTSS()" Also, the parameter "readsPerPiece" is at default set to 1000000. This will limit the memory the machine will use so that bigger files can still be read in. It also has a value, 'auto', which will determine how much memory the machine has and then give a value that fits that. I would not suggest using 'auto' with multicore, though.
	
        getTSS(myTSSr, useMultiCore = FALSE, numCores = NULL)
